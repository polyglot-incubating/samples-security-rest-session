...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    // @formatter:off
    http.csrf().disable().cors().configurationSource(cors).and().anonymous()
        .authenticationFilter(new AnonymousStatelessAuthenticationFilter(UUIDGenerator.uuid()))
        .and().httpBasic().disable().headers()
        .addHeaderWriter(new StaticHeadersWriter("X-Content-Security-Policy", "script-src 'self'"))
        .frameOptions().disable().and().authorizeRequests().antMatchers("/api/**").authenticated()
        .antMatchers(HttpMethod.POST, "/identity/**").permitAll().antMatchers("/admin/**")
        .hasRole("ADMIN").anyRequest().permitAll().and().requestCache()
        .requestCache(new NullRequestCache()).and().sessionManagement()
        .sessionCreationPolicy(SessionCreationPolicy.IF_REQUIRED).maximumSessions(2)
        .sessionRegistry(sessionRegistry).and().and().securityContext().and().servletApi().and()
        .authenticationProvider(authenticationProvider())
        .addFilter(new WebAsyncManagerIntegrationFilter()).logout();
    // @formatter:on
  }

  @Bean
  public UserPrincipalResolver userPrincipalResolver() {
    final JdbcUserPrincipalResolver userPrincipalResolver =
        new JdbcUserPrincipalResolver(jdbcTemplate);

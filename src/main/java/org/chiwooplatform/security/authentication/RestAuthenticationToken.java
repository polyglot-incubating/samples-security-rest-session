...
public class RestAuthenticationToken
    ...
    /**
     * @param principal This is can be used to represent any entity, such as an individual, a corporation, and a login
     *            id.
     ...
     */
    public RestAuthenticationToken( Object principal, Object credentials, String token, String... authorities ) {
        super( AuthorityUtils.createAuthorityList( authorities ) );
        // Arrays.asList( authorities ).stream().map( ( role ) -> new SimpleGrantedAuthority( role ) ).collect( Collectors.toList() )
        this.principal = principal;
        this.credentials = credentials;
        this.token = token;
        super.setAuthenticated( true );

...
public class OptionalTest {
    ...
    public final void ut1001() {
        ...
        log.info("optinal.map(s -> s + changed).orElse('none'): {}", optional.map(s -> s + " changed").orElse("none"));
    ...
    public final void ut1002() {
        ...
        log.info("optinal.map(s -> s + changed).orElse('none'): {}", optional.map(s -> s + " changed").orElse("none"));
        optional.ifPresent(
                (s) -> System.out.println("optional.ifPresent(): " + s.charAt(0) + "-" + s.charAt(s.length() - 1)));
    }
